...
// TypeScript port of the legacy app.js — DOM output, class names, and
// escaping/sanitizing behavior are unchanged. See migration plan §Faz 3.
// ===================================================================

import { escapeHtml, sanitizeUrl, sanitizeImgSrc } from "./lib/sanitize";
import { fetchPortfolioFromApi, fetchPortfolioStatic } from "./lib/api";
...
async function loadDynamicData(): Promise<void> {
  // 1. Try the backend API (same-origin, nginx proxies /api/ to the backend container)
  try {
    portfolioData = await fetchPortfolioFromApi();
    applyDynamicData();
    return;
  } catch (e) {
    console.log("Could not reach backend API, trying local/fallback options.", e);
  }

  // 2. Static fallback copy shipped alongside the site
  try {
    portfolioData = await fetchPortfolioStatic();
    applyDynamicData();
    return;
  } catch (e) {
    console.log("Static fallback also failed, trying localStorage.", e);
  }
...
function applyDynamicData(): void {
  ...
  if (portfolioData.personal) {
    ...
    if (channelsContainer) {
      const safeEmail = escapeHtml(p.email || "info@alarasysn.com");
        ...
        channelsHtml += `
          <a href="${safeInsta}" target="_blank" rel="noopener noreferrer" class="contact-button-card">
            ...
            <div class="contact-card-text">
              ...
              <span class="contact-card-value">@alarasysn</span>
            </div>
          </a>
        `;
      }
...
// NOTE: #theme-toggle does not exist in index.html — this listener never
// attaches and dark mode stays unreachable from the UI. That is the
// existing production behavior and is intentionally left as-is
// (see migration plan §Faz 10).
...
function initNavigation(): void {
  ...
  // NOTE: no null-guard on #hamburger, matching legacy behavior exactly
  // (see migration plan §Faz 10 — intentionally not "fixed").
...
function initPortfolioModal(): void {
  ...
  const carouselPrev = document.getElementById("carousel-prev") as HTMLElement;
  const carouselNext = document.getElementById("carousel-next") as HTMLElement;
  const carouselIndicators = document.getElementById("carousel-indicators") as HTMLElement;
  ...
  portfolioCards.forEach((card) => {
    card.addEventListener("click", () => {
      ...
      if (clientLabelValue && clientLabel) {
...
function initContactForm(): void {
  ...
  form.addEventListener("submit", (e) => {
    ...
    const phoneNum =
      portfolioData && portfolioData.personal && portfolioData.personal.phone
