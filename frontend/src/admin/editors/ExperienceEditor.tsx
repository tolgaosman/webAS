...
function AccomplishmentsField({ value, onChange }: { value: LocalizedString[]; onChange: (v: LocalizedString[]) => void }) {
  ...
  const handleChange = (raw: string) => {
    ...
    onChange(
      lines.map((line, i) => {
        ...
        // en/nl survive. Edited/new line: reset en/nl blank so the
        // backend's auto-translate (Translatable::set()) knows to
        // re-translate from the new text instead of trusting stale
        // leftovers (these rows are deleted and recreated on every save).
  ...
  return (
    <div className="form-group" style={{ marginTop: "1rem" }}>
      <label>Accomplishments & Responsibilities — one item per line</label>
